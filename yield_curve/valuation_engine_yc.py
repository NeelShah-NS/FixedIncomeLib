...
class ValuationEngineProductOvernightIndexCashflow(ValuationEngine):
    def __init__(
        ...
        fixings_file = valuation_parameters.get("fixings_file", "fixings/fixings.csv")
        IndexManager.instance().load_fixings_from_csv(fixings_file)

    def calculateValue(self):
        fixing_registry      = IndexManager.instance()
        realized_end_date    = min(self.valuation_date, self.termination_date)
        historical_fixings   = fixing_registry.get_fixings(
            self.index_name,
            self.effective_date,
            realized_end_date
        )
